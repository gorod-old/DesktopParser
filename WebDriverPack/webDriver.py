...
def try_func(func):
    def wrapper(*args, **kwargs):
        try:
            ...
            print_exception_msg(location=f'{func}', msg=f'@try_func - some error')
    return wrapper
...
class WebDriver(Parser):
    ...
    def _get_driver(self):
        ...
        while True:
            try:
                ...
                self.print_msg(location='WebDriver _get_driver()', msg=f'{e}', exception=True, stream=self.stream)
    ...
    def print_msg(self, location: str = None, msg: str = '', exception: bool = False, stream: int = None):
        if exception and self.except_print:
            print_exception_msg(location, msg, stream)
        else:
            print_info_msg(location, msg, stream)
